...
class DirectionalTraj(Node):
    ...
    def publish_traj(self,
        ...
        dz:float = self.target_state[2] - self.enu_state[2]
    ...
    def target_state_callback(self, msg: mavros.local_position.Odometry) -> None:
        """
        Converts NED to ENU and publishes the trajectory
          """
        self.target_state[0] = msg.pose.pose.position.x
        self.target_state[1] = msg.pose.pose.position.y
        ...
        self.target_state[3] = roll
        self.target_state[4] = pitch
        self.target_state[5] = yaw  # (yaw+ (2*np.pi) ) % (2*np.pi);

        vx = msg.twist.twist.linear.x
        vy = msg.twist.twist.linear.y
        vz = msg.twist.twist.linear.z
        # get magnitude of velocity
        self.target_state[6] = np.sqrt(vx**2 + vy**2 + vz**2)
...
def main(args=None):
    ...
    xF: np.array = traj_node.target_state
